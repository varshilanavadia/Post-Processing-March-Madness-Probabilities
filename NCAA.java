...
public class NCAA {
    public static void main(String[] args) {
		String year = "2017";
		int type = 1;           // SEE DEFINITION OF TYPE BELOW ON LINE 50
		int readFromFile = 1;
		...
		double[] approxTrueProb = calculateApproxTrueProb(year, predictions, uniqueMatchUpIDs, type, readFromFile, curDir);
		
		simulate(year, totalIterations, type, approxTrueProb, uniqueMatchUpIDs, predictions, seeds, slots, curDir);

	}

	private static void simulate(String year, int totalIterations, int type, double[] approxTrueProb, String[] uniqueMatchUpIDs, double[][] predictions, String[][] seeds, String[][] slots, String curDir) {
	...
	System.out.println("Simulation started");
	    ...
	    if (iteration % 1000 == 0) {
            ...
            String[][] tournamentResults = simulateTournament(year, tourneyStartSeedsAndIDs, uniqueMatchUpIDs, approxTrueProb, slots);
        ...
        System.out.println("Simmulation Complete!");
	...
	private static void getTrueProbRanksFromLeaderboard(String year, int type, String curDir) {
		System.out.println("Executing getTrueProbRanksFromLeaderboard()");
		int rank;
        ...
        List<Integer> trueProbRank = new ArrayList<Integer>();
	...
	private static String[][] simulateTournament(String year, String[][] tourneyStartSeedsAndIDs, String[] uniqueMatchUpIDs, double[] approxTrueProb, String[][] slots) {
        ...
        while (tournamentResults[tournamentResults.length - 1][tournamentResults[0].length - 1] == null) {
            for (int rowIndex = 0; rowIndex < tournamentResults.length; rowIndex++) {
                ...
                String team1 = Double.parseDouble(tournamentResults[rowIndex][1]) < Double.parseDouble(tournamentResults[rowIndex][2]) ? tournamentResults[rowIndex][1] : tournamentResults[rowIndex][2];
                String team2 = Double.parseDouble(tournamentResults[rowIndex][1]) < Double.parseDouble(tournamentResults[rowIndex][2]) ? tournamentResults[rowIndex][2] : tournamentResults[rowIndex][1];
                // THIS FOR LOOP IS POTENTIALLY DUPLICATED.
                for (int j = 0; j < uniqueMatchUpIDs.length; j++) {
                    if (uniqueMatchUpIDs[j].equals(teamID)) {
                        ...
                        if (coinFlip <= approxTrueProb[j]) {
                            tournamentResults[rowIndex][3] = team1;
                            tournamentResults[rowIndex][6] = String.valueOf(1);
                        } else {
                            tournamentResults[rowIndex][3] = team2;
	...
	private static double[] calculateApproxTrueProb(String year, double[][] predictions, String[] uniqueMatchUpIDs, int type, int readFromFile, String curDir) {
